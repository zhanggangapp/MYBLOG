    ...
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            try
            {
                ...
                using (var sw = new StreamWriter(@"D:\View\MYBLOG\CpuAnalyzer\bin\Debug\"+process.ProcessName + ".htm", false,
                                                 Encoding.Default))
                {
                    sw.Write(sb.ToString());
                }

                Process.Start(@"D:\View\MYBLOG\CpuAnalyzer\bin\Debug\"+process.ProcessName + ".htm" );
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
